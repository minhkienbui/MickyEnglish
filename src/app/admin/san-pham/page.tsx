...
export default function AdminProductsPage() {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (editingProductId) {
      ...
      updateProductAction(editingProductId, payload);
    } else {
      const generatedId = "prod-custom-" + Date.now();
      addProduct({ ...payload, id: generatedId });
      createProductAction({ ...payload, id: generatedId });
    }
            ...
            <div className="mt-6 border-t border-line pt-5">
              ...
              <div className="mt-4 grid grid-cols-2 gap-2">
                <Button variant="secondary" onClick={() => {
                  const nextStock = selectedProduct.stock - 1;
                  updateProductStock(selectedProduct.id, nextStock);
                  updateProductStockAction(selectedProduct.id, nextStock);
                }}>
                  -1
                </Button>
                <Button variant="secondary" onClick={() => {
                  ...
                  {badgeOptions.map((badge) => {
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        onClick={() => {
                          const nextBadges = active
                            ? selectedProduct.badges.filter((item) => item !== badge)
                            : [...selectedProduct.badges, badge];
                          updateProductBadges(selectedProduct.id, nextBadges);
                          updateProductBadgesAction(selectedProduct.id, nextBadges);
                        }}
