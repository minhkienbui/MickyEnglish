...
import { loginAction, registerAction, deleteRegisteredUserAction } from "@/lib/auth-actions";
  ...
  persist(
    (set, get) => ({
      ...
      deleteRegisteredAccount: (accountId) => {
        ...
        if (accountToDelete) {
          deleteRegisteredUserAction(accountToDelete.email);
          if (accountToDelete.customerId) {
